import os
import sys
# Enforce strict WebRTC blocking flags and absolute Widevine library target path before module parsing
os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = "--no-sandbox --disable-gpu-sandbox --disable-webrtc --disable-webrtc-hw-decoding --disable-webrtc-hw-encoding --disable-features=WebRtcHideLocalIpsWithMdns,WebRTC-MDNS-Negotiation --ignore-gpu-blocklist --enable-gpu-rasterization --enable-zero-copy --enable-widevine-cdm --widevine-path=/usr/lib/qt6/plugins/ppapi/libwidevinecdm.so"
...
class CleanRequestInterceptor(QWebEngineUrlRequestInterceptor):
    def interceptRequest(self, info):
        ...
        if "netflix.com" in url_string or "accounts.google.com" in url_string or "googleusercontent.com" in url_string:
            chrome_linux_agent = b"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
            info.setHttpHeader(b"User-Agent", chrome_linux_agent)
...
class CleanBrowserWindow(QMainWindow):
    def __init__(self):
        ...
        secure_agent = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
